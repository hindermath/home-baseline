...
- `scripts/invoke-psscriptanalyzer.ps1`: analyzes every Git-tracked PowerShell script, module, and data file with the pinned PSScriptAnalyzer version and repository settings.
...
- PSScriptAnalyzer `1.25.0` ist ein Required-PowerShell-Modul aus `scripts/config/powershell-modules-registry.json`; alle getrackten `.ps1`, `.psm1` und `.psd1` muessen den gemeinsamen Analyselauf bestehen. / PSScriptAnalyzer `1.25.0` is a required module; every tracked PowerShell file must pass the shared analysis run.
...
- `docs/secure-development/baseline-manifest.json` ist die kanonische Dateiliste fuer Basis 3.0.0; die Einzelchecklisten sind Quelle und der Sammelband wird mit `build-secure-development-docs.*` erzeugt und geprueft.
...
- Optional: `autonomous-run-governance` v0.1.2 prio 70 wird nur bei explizit delegierten vollständigen Läufen über `scripts/config/spec-kit-autonomous-governance-presets.json` ergänzt. Die Standard-Sechsermatrix bleibt unverändert; Installation erteilt keine Remote-, Merge-, Bypass- oder Provider-Rechte und `LocalImplementation` bleibt Default. Dokumentations-, Status-, Schema- oder Evidence-Änderungen gelten erst dann als testfrei, wenn keine ausführbaren Validatoren die geänderten Pfade, Marker, Schemas oder Zustandswerte konsumieren. Vor autorisierten Commits wird der exakt beabsichtigte Kandidat mit `git diff --cached --check` und Statusabgleich geprüft; fremde Änderungen bleiben unberührt. Vor einem Merge wird jeder Acceptance-Gate dem tatsächlich ausgeführten Workflow, Job, Runner beziehungsweise der Plattform und dem Befehl zugeordnet; grüne Namen oder ein Bypass ersetzen keinen technischen Nachweis.
