...
- Optionales Preset-Profil: `intake-review-governance` v0.1.0 prio 65 bleibt ausserhalb der Standard-Achtermatrix. Die verwaltete Thorsten-Flotte waehlt es ueber das Registry-Profil `intake-review-nine-governance-presets`; diese lokale Flottenwahl macht Preset 9 nicht zum allgemeinen Default. Neue registrierte Flotten-Repositories erben `defaultPresetProfile`. Bei aktiver Projekt- oder Kampagnenpolicy werden nur aktuelle `Ready`- oder menschlich akzeptierte `ReadyWithAcceptedRisks`-Ergebnisse zugelassen; Critical/High, offene materielle Fragen, Hash-Drift oder fehlende Worker-Coverage blockieren. Review und Status sind read-only, Repair benoetigt ausdrueckliche Aenderungsautoritaet. Lernreihen duerfen Intake Review vorbereiten und paketieren, starten aber keinen Lernendenlauf ohne ausdruecklichen Auftrag. *Optional preset profile: `intake-review-governance` v0.1.0 at priority 65 remains outside the standard eight. Thorsten's managed fleet selects it through `intake-review-nine-governance-presets`; this local fleet choice does not change the public default. New registered fleet repositories inherit `defaultPresetProfile`. Active policies accept only current `Ready` or human-approved `ReadyWithAcceptedRisks` results; learner runs still require explicit authorization.*
