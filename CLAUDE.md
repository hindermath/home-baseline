...
- Optionales Preset-Profil: `intake-review-governance` v0.1.0 prio 65 prueft einzelne Intakes, Reihen und Kampagnen vor der Feature-Erstellung. Es bleibt ausserhalb der Standard-Achtermatrix; bei aktiver Projekt- oder Kampagnenpolicy werden nur aktuelle `Ready`- oder menschlich akzeptierte `ReadyWithAcceptedRisks`-Ergebnisse zugelassen. Critical/High, offene materielle Fragen, Hash-Drift oder fehlende Worker-Coverage blockieren. Review und Status sind read-only; Repair benoetigt ausdrueckliche Aenderungsautoritaet. *Optional preset profile: `intake-review-governance` v0.1.0 at priority 65 reviews single intakes, series, and campaigns before feature creation. It remains outside the standard eight; an active policy accepts only current `Ready` or human-approved `ReadyWithAcceptedRisks` results.*
