...
# GIT-SCOPE-002: [includeIf "gitdir:~/home-baseline-tmp/"] present in ~/.gitconfig
...
hg_check_git_scope() {
  ...
  # GIT-SCOPE-002: includeIf für home-baseline-tmp in ~/.gitconfig
  if ! grep -qF 'gitdir:~/home-baseline-tmp/' "$gitconfig" 2>/dev/null; then
    if [ "${OPT_JSON:-false}" = "true" ] || [ "${1:-}" = "--json" ]; then
