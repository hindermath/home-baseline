# Lastenheft: Secure ServiceHarvester v2 06 - Fehler-, Validierungs- und Response-Modell
...
**DE:** Einheitliche Validierung, fachliche Fehler, technische Fehler und ein sicheres Fehler- und Response-Modell werden getrennt festgelegt. Import aus dem Harvester und Anfragen an die kleine API teilen sich denselben Vertrag, ohne Interna preiszugeben.

**EN:** Unified validation, business errors, technical errors, and a safe error and response model are defined separately. Harvester import and requests to the small API share the same contract without exposing internals.
...
<!-- lf-konkret -->
**DE:** Konkrete Lernfelder (Quelle: `Rahmenlehrplan-Lernfeld-Mapping.md`, Abschnitt Secure ServiceHarvester v2): **primär LF 5 („Software zur Verwaltung von Daten anpassen")**; berührt LF 4 („Schutzbedarfsanalyse im eigenen Arbeitsbereich durchführen"). Vertiefte Erklärung und Verständnisfragen: `lernbegleiter/Secure-ServiceHarvester-v2_06_Fehler-Validierungs-und-Response-Modell.Lernbegleiter.md`.

**EN:** Concrete learning fields (source: `Rahmenlehrplan-Lernfeld-Mapping.md`, section Secure ServiceHarvester v2): **primary LF 5 ("Software zur Verwaltung von Daten anpassen")**; touched LF 4 ("Schutzbedarfsanalyse im eigenen Arbeitsbereich durchführen"). Deeper explanation and comprehension questions: `lernbegleiter/Secure-ServiceHarvester-v2_06_Fehler-Validierungs-und-Response-Modell.Lernbegleiter.md`.
...
**DE:** Erarbeite für ein einheitliches Fehler-, Validierungs- und Response-Modell des Secure ServiceHarvester eine fokussierte Spezifikation. Berücksichtige: eine gemeinsame Validierung für Harvester-Import und API, die Trennung von Validierungs-, fachlichen und technischen Fehlern, sichere Rückmeldungen ohne Interna und Negativtests. Die C#-Referenz `InventarWorkerService` dient nur zur Orientierung.

**EN:** Prepare a focused specification for a unified error, validation, and response model of the Secure ServiceHarvester. Consider a shared validation for harvester import and API, the separation of validation, business, and technical errors, safe responses without internals, and negative tests. The C# reference `InventarWorkerService` serves only as orientation.
...
- Harvester-Import, API-Eingaben, Konfiguration, Logs und Abhängigkeiten als mögliche Vertrauensgrenzen prüfen.
...
- Erwartete Evidenz: Fehlertabelle für Import und API, Negativtests, sichere Meldungen, Logging-Grenzen.
