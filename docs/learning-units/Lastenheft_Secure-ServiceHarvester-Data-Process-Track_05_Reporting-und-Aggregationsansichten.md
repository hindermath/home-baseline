# Lastenheft: Secure ServiceHarvester Data & Process Track 05 - Reporting und Aggregationsansichten
...
**DE:** Aus den definierten Kennzahlen (Einheit 03) entstehen barrierefreie Reporting- und Aggregationsansichten. Lernende wählen begründete Aggregationsebenen (z. B. Betriebssystem, Standort, Zeitfenster), gestalten Berichte adressatengerecht, übernehmen die Aussagegrenzen jeder Kennzahl in den Bericht und stellen sicher, dass Aggregation zugleich Datenschutz stärkt und keine Einzelperson nachverfolgbar macht.

**EN:** From the defined metrics (unit 03), accessible reporting and aggregation views are created. Learners choose justified aggregation levels (e.g. operating system, site, time window), design reports for their audience, carry the interpretation limits of every metric into the report, and ensure that aggregation also strengthens privacy and makes no individual traceable.

## Fachrichtungsbezug / Specialization Fit

| Fachrichtung | Eignung | Reflexionsfrage |
|---|---|---|
| DPA | hoch (Schwerpunkt) | Welche Aggregationsebene beantwortet die Kundenfrage, ohne Einzelpersonen sichtbar zu machen? |
| AE | mittel | Welche Abfrage-, Gruppierungs- oder Darstellungslogik folgt aus der Reporting-Entscheidung? |
| SI | mittel | Welche Betriebslast oder Aktualität begrenzt die Berichtserzeugung? |
...
<!-- lf-konkret -->
**DE:** Konkrete Lernfelder (Quelle: `Rahmenlehrplan-Lernfeld-Mapping.md`): **primär LF 11c („Prozesse analysieren und gestalten")**; berührt LF 6 („Serviceanfragen bearbeiten"). Begründung: Reporting- und Aggregationsansichten gestalten den Auswertungsprozess, aus dem konkrete Serviceanfragen und Wartungsentscheidungen entstehen. Vertiefte Erklärung und Verständnisfragen: `lernbegleiter/Secure-ServiceHarvester-Data-Process-Track_05_Reporting-und-Aggregationsansichten.Lernbegleiter.md`.

**EN:** Concrete learning fields (source: `Rahmenlehrplan-Lernfeld-Mapping.md`): **primary LF 11c ("Prozesse analysieren und gestalten")**; touched LF 6 ("Serviceanfragen bearbeiten"). Rationale: reporting and aggregation views shape the evaluation process from which concrete service requests and maintenance decisions arise. Deeper explanation and comprehension questions: `lernbegleiter/Secure-ServiceHarvester-Data-Process-Track_05_Reporting-und-Aggregationsansichten.Lernbegleiter.md`.
...
**DE:** Erarbeite für Reporting und Aggregationsansichten eine fokussierte Spezifikation für die Spezialisierung DPA - Daten- und Prozessanalyse. Berücksichtige: die Auswahl begründeter Aggregationsebenen (z. B. je Betriebssystem, Standort oder Zeitfenster), die adressatengerechte Gestaltung je Bericht (wer liest ihn und für welche Entscheidung), die ausdrückliche Übernahme der Aussagegrenzen aus Einheit 06 in jeden Bericht, Aggregation als Datenschutzmaßnahme (keine Einzelperson nachverfolgbar, sinnvolle Mindestgruppengröße) sowie barrierefreie Darstellung (echte Tabellen mit Kopfzeile, keine reine Farbcodierung). Baue auf den Kennzahlen aus Einheit 03 auf. Nutze die C#-Referenz `InventarWorkerService` nur als Vergleich; die Lösung bleibt sprachneutral.

**EN:** Prepare a focused specification for reporting and aggregation views for the DPA - Data and Process Analysis specialization. Consider: the choice of justified aggregation levels (e.g. per operating system, site, or time window), audience-appropriate design per report (who reads it and for which decision), the explicit carrying-over of the interpretation limits from unit 06 into every report, aggregation as a privacy measure (no individual traceable, sensible minimum group size), and accessible presentation (real tables with a header row, no color-only coding). Build on the metrics from unit 03. Use the C# reference `InventarWorkerService` only for comparison; the solution stays language-neutral.
...
- Aggregation so wählen, dass keine Einzelperson und keine Einzelmaschine über kleine Gruppen re-identifizierbar wird.
...
- [ ] Aggregation schützt nachweislich vor Re-Identifikation einzelner Personen oder Maschinen.
...
```text
/speckit-specify Nutze docs/learning-units/Lastenheft_Secure-ServiceHarvester-Data-Process-Track_05_Reporting-und-Aggregationsansichten.md als verbindliche Eingabedatei. Erstelle eine fokussierte Feature-Spezifikation für Reporting und Aggregationsansichten der EuFPA-Lernreihe Secure ServiceHarvester Data & Process Track. Erzeuge keine Implementierung und starte keinen Sammellauf für die gesamte Lernreihe.
```
