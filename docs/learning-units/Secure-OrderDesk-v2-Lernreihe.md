# Secure OrderDesk v2 Lernreihe / Secure OrderDesk v2 Learning Series
...
**DE:** Diese Datei beschreibt die **Aufbaureihe v2 (2. Lehrjahr)** der Familie **Secure OrderDesk**. Sie setzt die [Basis-Reihe (Jahr 1)](Secure-OrderDesk-Lernreihe.md) fort und bereitet die [Professional Tracks (Jahr 3)](Secure-OrderDesk-Professional-Tracks-Lernreihe.md) vor. Das Arbeiten mit KI-Agenten und Spec Kit bleibt der durchgehende methodische Faden.

**EN:** This file describes the **advanced series v2 (year 2)** of the **Secure OrderDesk** family. It continues the [base series (year 1)](Secure-OrderDesk-Lernreihe.md) and prepares the [Professional Tracks (year 3)](Secure-OrderDesk-Professional-Tracks-Lernreihe.md). Working with AI agents and Spec Kit remains the continuous methodical thread.

## Zielbild / Target Picture

**DE:** v2 vertieft den Secure OrderDesk vom einfachen Bestell-Programm zu einer sauber geschnittenen Handelsplattform: klare Systemgrenzen zwischen Bestellannahme, Katalog, relationaler Ablage und Schnittstelle, plattformübergreifendes Hosting, eine austauschbare Backend-Abstraktion, sichere SQL-Zugriffe und beobachtbarer Betrieb. Der fachliche Kernvertrag aus der Basis-Reihe – die Northwind-Bestelldomäne für die Kundenfirma Secure Trader – bleibt gültig und wird technisch tiefer umgesetzt.

**EN:** v2 deepens the Secure OrderDesk from a simple ordering program into a cleanly separated trading platform: clear system boundaries between order intake, catalog, relational store, and interface, cross-platform hosting, a replaceable backend abstraction, secure SQL access, and observable operation. The shared core contract from the base series – the Northwind ordering domain for the client company Secure Trader – stays valid and is implemented at greater technical depth.
...
| SI – Systemintegration | Datenbank-Betrieb, Konfiguration, Secrets, Logging, Health, Monitoring und Betriebsnachweise. |
| DPA – Daten- und Prozessanalyse | Datenqualität in Bestellungen und Positionen, Umsatz-Kennzahlen, Aussagegrenzen und Datenschutz. |
| DV – Digitale Vernetzung | Systemgrenzen, Schnittstellen der Bestell-Pipeline, Kommunikationsflüsse und Verfügbarkeit. |

**EN:** Application Development focuses on architecture, backend abstraction, API contract, error/response model, and tests. System Integration focuses on database operation, configuration, secrets, logging, health, monitoring, and operational evidence. Data and Process Analysis focuses on order and line-item data quality, revenue metrics, limits of interpretation, and privacy. Digital Networking focuses on system boundaries, order-pipeline interfaces, communication flows, and availability.

## Lernziele / Learning Goals

- Eine relationale Bestell-/Handelsplattform plattformübergreifend hosten und sicher durch ihren Lebenszyklus führen.
- Persistenz hinter einer Abstraktion kapseln und relationale Backends bewusst wechseln (SQLite, PostgreSQL, SQL Server).
- Bestellannahme und Katalogzugriffe idempotent, validiert und mit parametrisierten Queries gehärtet gestalten.
- Einen klaren API-/Service-Vertrag mit sauberer Fehlerbehandlung ohne interne Details anbieten.
- Betrieb beobachtbar machen: strukturiertes Logging, Health/Status, einfache Umsatz-/Bestell-Metriken, keine Secrets und keine Kundendaten im Klartext.

*English summary: learners practice hosting, persistence abstraction, hardened SQL access with parameterized queries, API contracts, and observable operation at second-year depth, with Spec Kit as the companion method.*

## Arbeitsmodell / Working Model

**DE:** Diese Reihe wird jetzt nur als kanonisches Level-0-Lernmaterial vorbereitet. Level-1- und Level-2-Repositories werden später separat angelegt. Die späteren Level-2-Pfade nutzen C#, Go, Java, Python, Rust und Swift und führen die Secure-Development-Basis lokal mit. Der empfohlene Startpunkt ist ein abgeschlossener Jahr-1-Stand (Tag `learning/year-1-complete`). Der Pflicht-Datensatz `ALFKI` bleibt in jedem Pfad prüfbar.

**EN:** This series is currently prepared only as canonical level-0 learning material. Level-1 and level-2 repositories will be created later in a separate step. Later level-2 paths use C#, Go, Java, Python, Rust, and Swift and carry the secure-development baseline locally. The recommended starting point is a completed year-1 state (tag `learning/year-1-complete`). The mandatory record `ALFKI` stays verifiable in every path.
...
| 02 | Plattformübergreifendes Hosting und Lebenszyklus | SI, DV, AE |
| 03 | Relationales Persistenzmodell und Backend-Abstraktion | AE, SI, DPA |
| 04 | Bestellannahme: Idempotenz und Import-Härtung | AE, DPA, SI |
