...
**DE:** Die öffentliche Basis für Level-2-Repos bleibt das zentrale Acht-Preset-Profil. Thorstens registrierte Lernreihen-Flotte wählt zusätzlich Model Routing, Intake Authoring, Intake Review und Intake Sequencing über `model-routing-twelve-governance-presets`. Model Routing ordnet auf Priorität 61 pro Rechner erkannte Modelle providerneutralen Rollen zu; konkrete Modelle bleiben lokal. Authoring verwaltet auf Priorität 64 Intakes und ausdrücklich freigegebene Serien; Review prüft auf 65 Inhalte und Hashes unabhängig; Sequencing verwaltet auf 66 Reihenfolge und Abhängigkeiten. Die Installation erteilt keine Ausführungs- oder Delivery Authority. Kein Lernendenlauf startet ohne ausdrücklichen Auftrag.

**EN:** The public baseline for level-2 repositories remains the central eight-preset profile. Thorsten's registered learning-series fleet additionally selects Model Routing, Intake Authoring, Intake Review, and Intake Sequencing through `model-routing-twelve-governance-presets`. Model Routing at priority 61 maps models discovered on each machine to provider-neutral roles; concrete models remain local. Authoring at 64 manages intakes and explicitly approved series; Review at 65 independently validates content and hashes; Sequencing at 66 manages order and dependencies. Installation grants no execution or delivery authority. No learner run starts without explicit authorization.
