# Lernbegleiter: Secure OrderDesk Digital Networking Track 01 – Track-Baseline und Netzzielbild / Study Companion

**Stand / Date:** 2026-07-07
**Zugehöriges Lastenheft / Related intake:** `Lastenheft_Secure-OrderDesk-Digital-Networking-Track_01_Track-Baseline-und-Netzzielbild.md`
...
**DE:** Das Secure OrderDesk ist die Bestell- und Handelsplattform der fiktiven Kundenfirma Secure Trader. Sie
arbeitet mit der relationalen Northwind-Datenbasis, in der der Beispielkunde `ALFKI` (Alfreds Futterkiste)
fest verankert ist. In den Jahren 1 und 2 (v2) hast du die Plattform selbst gebaut: Domänenmodell, Validierung,
relationale Persistenz, API, Betrieb. Jetzt, im DV-Track des 3. Lehrjahres, wechselt der Blick von *einer*
Anwendung auf das *Netz*, in dem die Bestell-Pipeline verteilt arbeitet. Die erste Einheit legt die
**Track-Baseline** fest (was aus v2 vorhanden ist) und übersetzt sie in ein **Netzzielbild**: Welche
Schutzziele hat das Netz? Wo liegen grobe Vertrauenszonen? Welche Verfügbarkeit ist gefordert?

**EN:** The Secure OrderDesk is the ordering and trading platform of the fictional client company Secure Trader.
It works with the relational Northwind data, in which the sample customer `ALFKI` (Alfreds Futterkiste) is
firmly anchored. In years 1 and 2 (v2) you built the platform yourself: domain model, validation, relational
persistence, API, operations. Now, in the Digital Networking Track of year 3, the focus shifts from *one*
application to the *network* in which the order pipeline works in a distributed way. This first unit records
the **track baseline** (what exists from v2) and translates it into a **network target picture**: which
protection goals does the network have? Where are coarse trust zones? Which availability is required?
...
einen langlaufenden Dienst; die Aufgabe bleibt sprachneutral.
...
example of a long-running service; the task stays language-neutral.
...
| Netzzielbild / Network target picture | Beschreibung, was das Netz der Bestell-Pipeline leisten soll und welche Sicherheit angestrebt wird. |
| Bestell-Pipeline / Order pipeline | Der vernetzte Weg einer Bestellung von der Annahme über den Katalog bis in die relationale Ablage. |
...
**DE:** **Schritt 1 – Track-Baseline festhalten.** Schreibe kurz auf, was aus v2 vorliegt: eine
Bestell-/Handelsplattform mit Domänenmodell, Validierung, relationaler Persistenz (Northwind-Schema) und API.
Diese Baseline ist der Startpunkt; du entwickelst sie nicht neu, sondern vernetzt sie sicher.

**EN:** **Step 1 – Record the track baseline.** Briefly write down what exists from v2: an ordering/trading
platform with domain model, validation, relational persistence (Northwind schema), and API. This baseline is
the starting point; you do not rebuild it, you network it securely.

**DE:** **Schritt 2 – Netz-Schutzziele benennen.** Ordne den drei Schutzzielen konkrete Aussagen zu.
Vertraulichkeit: Wer darf Kunden- und Bestelldaten sehen? Integrität: Warum wäre eine gefälschte Bestellung
gefährlich? Verfügbarkeit: Was passiert, wenn die Bestellannahme nicht erreichbar ist? Trenne die drei
bewusst, weil sie verschiedene Maßnahmen nach sich ziehen.

**EN:** **Step 2 – Name network protection goals.** Assign concrete statements to the three protection goals.
Confidentiality: who may see customer and order data? Integrity: why would a forged order be dangerous?
Availability: what happens if order intake is unreachable? Separate the three deliberately, because they lead
to different measures.

**DE:** **Schritt 3 – Grobe Vertrauenszonen skizzieren.** Teile das Bild in wenige Zonen: Bestell-Clients, die
Bestellannahme, den Katalog, die relationale Ablage und die Auswertung. Es geht noch nicht um genaue Regeln,
sondern um die Frage: Wo wechselt Vertrauen? Diese groben Zonen leiten die spätere Segmentierung (Einheit 04).

**EN:** **Step 3 – Sketch coarse trust zones.** Divide the picture into a few zones: order clients, order
intake, the catalog, the relational store, and analytics. It is not yet about exact rules, but about the
...
ein einfaches, messbares Ziel fest, etwa: „Die Bestellannahme ist zu 99 % der Geschäftszeit erreichbar; eine
verpasste Bestellung wird später zuverlässig nachverarbeitet." Solche Ziele machen spätere
Resilienzentscheidungen begründbar.

**EN:** **Step 4 – Set availability targets.** A network is only useful if it is reliable. Define a simple,
measurable target, e.g.: "order intake is reachable during 99 % of business hours; a missed order is reliably
post-processed later." Such targets make later resilience decisions justifiable.

**DE:** **Schritt 5 – Scope, Nicht-Ziele und Nachweise trennen.** Sage klar, was der DV-Track behandelt
(sichere Vernetzung der Bestell-Pipeline) und was nicht (z. B. neue fachliche Bestellfelder oder UI-Redesign).
Für jede Sicherheits- oder Verfügbarkeitsaussage nennst du einen Nachweispfad. Nicht anwendbare Standards
notierst du als `N/A` mit kurzer Begründung, statt sie wegzulassen.

**EN:** **Step 5 – Separate scope, non-goals, and evidence.** State clearly what the Digital Networking Track
covers (secure networking of the order pipeline) and what not (e.g. new functional order fields or UI
redesign). For each security or availability claim, name an evidence path. Non-applicable standards are noted
as `N/A` with a short justification instead of being omitted.
...
```text
Track-Baseline (v2):  Bestell-/Handelsplattform mit Modell, Validierung, relationaler Persistenz (Northwind), API (Referenz: InventarWorkerService)

Netz-Schutzziele:
  Vertraulichkeit = hoch    (personenbezogene Kundendaten wie ContactName in Customers)
  Integritaet     = hoch    (falsche Bestellung -> falsche Lieferung/Abrechnung)
  Verfuegbarkeit  = hoch    (Bestellannahme muss zuverlaessig erreichbar sein)

Grobe Vertrauenszonen: [Bestell-Clients] -> [Bestellannahme] -> [Katalog] / [relationale Ablage] -> [Auswertung]
Verfuegbarkeitsziel:   Bestellannahme zu 99 % der Geschaeftszeit erreichbar; verpasste Bestellung wird nachverarbeitet
Nicht-Ziel:            neue fachliche Bestellfelder; UI-Redesign
Nachweis:              Verfuegbarkeits-Testprotokoll -> Folgeeinheiten
Beispielkunde:         ALFKI (Alfreds Futterkiste, Maria Anders, Berlin) - rein fiktiv
```
...
| LF 10d Cyber-physische Systeme entwickeln | Berührt / Touched | Das Netzzielbild ist die Grundlage der späteren Entwicklung der vernetzten Bestell-Pipeline. |
...
3. **DE:** Warum ist Integrität bei Bestelldaten oft ein besonders hohes Schutzziel? /
   **EN:** Why is integrity often an especially high protection goal for order data?

   <details><summary>Musterantwort / Model answer</summary>

   **DE:** Aus Bestelldaten werden Lieferung und Abrechnung abgeleitet. Eine gefälschte oder falsche Bestellung
   führt direkt zu falscher Lieferung oder Rechnung, daher zählt die Unverfälschtheit stark.
   **EN:** Delivery and billing are derived from order data. A forged or wrong order leads directly to wrong
   delivery or invoicing, so integrity matters strongly.
...
4. **DE:** (SI) Wie formuliert man ein Verfügbarkeitsziel so, dass es nachweisbar ist? /
   ...
   **DE:** Mit einer messbaren Größe und Zeitbezug, etwa „zu 99 % der Geschäftszeit erreichbar". Nachweis: ein
   Erreichbarkeits- oder Verfügbarkeitsprotokoll. Ohne Messgröße ist das Ziel nicht prüfbar.
   **EN:** With a measurable metric and time reference, e.g. "reachable during 99 % of business hours".
...
5. **DE:** (DPA) Welche Datenschutzfrage sollte schon im Netzzielbild geklärt werden? /
   ...
   **DE:** Ob und welche personenbezogenen Kundendaten (z. B. `ContactName`, Adresse) über das Netz fließen.
   Das entscheidet über Vertraulichkeitsziel, Verschlüsselung und engere Pfade in späteren Einheiten.
   **EN:** Whether and which personal customer data (e.g. `ContactName`, address) flow over the network. That
   decides the confidentiality goal, encryption, and tighter paths in later units.
...
- [ ] grobe Vertrauenszonen der Bestell-Pipeline skizzieren.
...
- [ ] sketch coarse trust zones of the order pipeline.
...
`Lastenheft_Secure-OrderDesk-Digital-Networking-Track_01_Track-Baseline-und-Netzzielbild.md`. Danach führt
Einheit 02 (`Systemkontext und Bestell-Pipeline-Netzbezüge`) das grobe Zielbild in einen genauen Systemkontext
weiter. Der Copy-Paste-Prompt für einen späteren, manuell gestarteten Spec-Kit-Lauf steht im Lastenheft.

**EN:** Once you understand this explanation, work on the related intake
`Lastenheft_Secure-OrderDesk-Digital-Networking-Track_01_Track-Baseline-und-Netzzielbild.md`. Afterwards, unit
02 (`system context and order-pipeline network relations`) develops the coarse target picture into a precise
system context. The copy-paste prompt for a later, manually started Spec Kit run is provided in the intake.
