# Lernbegleiter: Secure CaseTracker 09 – Sandbox und agentische Entwicklung / Study Companion

**Stand / Date:** 2026-07-05
**Zugehöriges Lastenheft / Related intake:** `Lastenheft_Secure-CaseTracker_09_Sandbox-und-Agentische-Entwicklung.md`
...
nicht beschädigen. Wenn KI-Agenten (Codex, Copilot, Claude, Gemini) beim Entwickeln helfen, sollen riskantere
Schritte in einer freigegebenen Sandbox wie `absdd-image-sandbox` ablaufen – mit klaren **Mounts**
(Schreibbereichen), **Schreibgrenzen**, **Netzwerkannahmen** und **Secret-Regeln**. In dieser Einheit planst
du, wie Secure-CaseTracker-Aufgaben später in oder mit dieser Sandbox bearbeitet werden. Wichtig: Das
**Container-First-Gate** gilt ab Unit 00 verbindlich — jeder KI-Agenten-Aufruf erfolgt im Container/der Sandbox,
**nie** auf dem Arbeitsplatz-Rechner (Grundlage `Secure-Trader-Sandbox-Preflight.md`). Nur agentenlose Arbeit —
allgemeine Entwicklung, Lesen und Review — darf außerhalb stattfinden, z. B. mit VS Code oder JetBrains-IDEs.
Diese Einheit vertieft die vollständige Sandbox-Profilierung.

**EN:** A **sandbox** is a fenced-off play area: what happens inside cannot damage the rest of the system. When
AI agents (Codex, Copilot, Claude, Gemini) help with development, riskier steps should run in an approved
sandbox like `absdd-image-sandbox` – with clear **mounts** (write areas), **write boundaries**, **network
assumptions**, and **secret rules**. In this unit you plan how Secure CaseTracker tasks can later be worked on
in or with this sandbox. Important: the **container-first gate** is binding from unit 00 — every AI-agent
invocation happens inside the container/sandbox, **never** on the workstation (basis
...
laufen sollen: riskantere Experimente, KI-Agentenläufe, reproduzierbare Toolchains. Warum? Die Sandbox ist das
Zielbild für Arbeit mit klaren Schreib- und Netzwerkgrenzen; einfaches Lesen und Review braucht sie nicht.

**EN:** **Step 1 – Plan sandbox classification.** Describe which tasks should later run in `absdd-image-sandbox`:
riskier experiments, AI agent runs, reproducible toolchains. Why? The sandbox is the target picture for work
with clear write and network boundaries; simple reading and review do not need it.

**DE:** **Schritt 2 – Mounts, Schreibgrenzen und Netzwerk festlegen.** Dokumentiere, welche Verzeichnisse
eingebunden werden, wo ein Agent schreiben darf und ob Netzwerkzugriff nötig ist. Netzwerk ist begrenzt oder
als Risikoentscheidung dokumentiert. Warum? Enge Grenzen schützen den Host: Ein Agent kann nur in freigegebenen
Bereichen Schaden anrichten.

**EN:** **Step 2 – Define mounts, write boundaries, and network.** Document which directories are mounted,
where an agent may write, and whether network access is needed. Network is limited or documented as a risk
decision. Why? Tight boundaries protect the host: an agent can only cause damage in approved areas.

**DE:** **Schritt 3 – Secret-Regeln und Datenschutz.** Secrets gehören nicht in Prompts, Logs, Screenshots
oder Projektdateien. Keine echten personenbezogenen Daten in Agentenläufe. Logs aus Sandbox- oder
Agentenläufen werden auf sensible Inhalte geprüft. Warum? KI-Agenten verarbeiten viel Text; ein Secret im
Prompt kann leicht in ein Log oder Cache gelangen.

**EN:** **Step 3 – Secret rules and privacy.** Secrets do not belong in prompts, logs, screenshots, or project
files. No real personal data in agent runs. Logs from sandbox or agent runs are checked for sensitive content.
Why? AI agents process a lot of text; a secret in a prompt can easily end up in a log or cache.
...
Secrets in Prompts oder Logs. Unbegrenzte Schreib- oder Netzwerkzugriffe. Echte personenbezogene Case-Daten in
Agentenläufen statt fiktiver Testdaten. Fehlende Toolchains verschweigen. Agenten-Regeln nur für einen Agenten
schreiben. Agenten-Caches ins Projekt committen.

**EN:** **Common mistakes.** Starting an AI agent directly on the workstation instead of inside the container.
Secrets in prompts or logs. Unlimited write or network access. Real personal case data in agent runs instead
of fictitious test data. Hiding missing toolchains. Writing agent rules for only one agent. Committing agent
caches to the project.
...
Secrets:        keine in Prompt/Log/Screenshot/Projektdatei
...
Gate:           Container-First ab Unit 00 verbindlich -> KI-Agent nur im Container, nie auf dem Arbeitsplatz-Rechner.
```
...
| LF 9 Netzwerke und Dienste bereitstellen | Berührt / Touched | Netzwerkannahmen und reproduzierbare Toolchains betreffen bereitgestellte Dienste. |
...
Schreib-, Netzwerk- und Secret-Grenzen und nur gegen fiktive Case-Testdaten; dieses Gate gilt ab Unit 00
verbindlich (in ISO-27001-zertifizierten Organisationen ein pruefbarer Kontrollpunkt, u. a. A.8.25, A.8.31).*
A11Y-Aspekt: Die MSL-Support-Matrix und die Agenten-Grenzen werden als Texttabellen mit klaren Statuswörtern
geführt, nicht nur farblich, damit sie mit Screenreader und Braille-Zeile lesbar bleiben.
...
boundaries and only against fictitious case test data; this gate is binding from unit 00 (an auditable control
point in ISO-27001-certified organizations, e.g. A.8.25, A.8.31).* Accessibility aspect: the MSL support matrix
and the agent boundaries are kept as text tables with clear status words, not only by color, so they remain
...
3. **DE:** (AE) Welche Projektdateien darf ein Agent verändern? /
   **EN:** (AE) Which project files may an agent modify?
...
5. **DE:** Warum gilt das Container-First-Gate schon ab Unit 00, auch wenn die Sandbox-Technik erst hier vertieft wird? /
...
6. **DE:** Warum müssen die Agenten-Grenzen für alle Agenten gleichwertig verständlich sein? /
...
`Lastenheft_Secure-CaseTracker_09_Sandbox-und-Agentische-Entwicklung.md`. Der Copy-Paste-Prompt für einen
späteren, manuell gestarteten Spec-Kit-Lauf steht dort.

**EN:** Once you understand this explanation, work on the related intake
`Lastenheft_Secure-CaseTracker_09_Sandbox-und-Agentische-Entwicklung.md`. The copy-paste prompt for a later,
