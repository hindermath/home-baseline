# delta: Level-0-Feldtest / Level-0 field test
...
aktueller genehmigter Scope. Prüfe ausschließlich das Gate delta mit dem
...
delta using the installed validator. Compare ordinal path/raw SHA-256 snapshots
before and after. Stop on drift, missing evidence or unexpected outcomes.
Review never changes sources, decisions or images.*

```bash
bash .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.sh review delta home-baseline-assurance-v013 training
```

Bash zuerst auf macOS, danach PowerShell mit Action Review, Gate delta,
...
```powershell
pwsh -NoProfile -File .specify/presets/secure-development-assurance-governance/scripts/validate-secure-development-assurance.ps1 -Action Review -Gate delta -ContextId home-baseline-assurance-v013 -Mode training
```
