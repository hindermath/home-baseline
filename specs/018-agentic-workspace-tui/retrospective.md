...
| AR-018-005 | Cross-platform test discovery must not treat a foreign shell launcher as a usable native runtime or interpret Unix permission bits on Windows. | test/workflow | The Bash and PowerShell wrappers are Home-Baseline entry points. | Gate executable behavior by the target operating system while retaining portable source-contract checks on every platform. | 1 | High | A detected but unusable compatibility launcher can create false failures, while a skipped native path can hide real regressions. | Run the same suite on Unix and Windows; execute Bash only on Unix, PowerShell behavior on Windows, and shared source/contract checks everywhere. | NoPromotion |
