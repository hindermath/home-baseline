...
# Lastenheft: Level-0-Dokumentations-Konsistenz-Remediation und Flotten-Governance
...
Dieser Intake behebt ausschließlich bestätigte zentrale `DOC###`-Findings aus
Vorlauf D1 im Level-0-Repository und führt einen dauerhaften
**Documentation-Impact-Vertrag** ein. Dieser Vertrag verlangt bei jeder
fachlichen oder technischen Änderung eine nachvollziehbare Entscheidung,
welche Dokumentation betroffen ist.
...
*This intake remediates only confirmed central D1 findings in Level 0 and
establishes a durable fleet documentation-impact contract across normative
policy, workflow templates, agent guidance, and verifiable evidence.*
...
Level-0/1/2-Audit aus D1. Er disponiert alle D1-Findings, behebt aber nur die
bestätigten zentralen Level-0-Findings. Er liefert die verbindliche Baseline und
das wiederverwendbare Prüfschema für die registrierten Level-1- und
Level-2-Repositories in Vorlauf D3.
...
- CI oder deterministische Validatoren, soweit Vorlauf D1 einen belastbaren
  automatisierbaren Vertrag nachweist.

## 5. Anforderungen / Requirements

- **DRG-001:** Alle bestätigten D1-Findings werden als `Resolved`,
  `AcceptedRisk`, `FollowUp` oder `N/A` dispositioniert und zusätzlich genau
  einer zentralen Level-0-Remediation oder einem repository-spezifischen
  Level-1/2-Handoff an D3 zugeordnet.
...
- **DRG-014:** Vorlauf D3 übernimmt nur die gemergte und validierte
  Level-0-Baseline sowie die ausdrücklich an D3 übergebenen
  repository-spezifischen Findings.
- **DRG-015:** Der Documentation-Impact-Vertrag erklärt die Zuständigkeit für
  Level 0, Level 1 und Level 2, ohne identische Dateien oder Richtlinientexte
  in allen Repositories zu erzwingen.
- **DRG-016:** D2 verändert keine Level-1-/Level-2-Arbeitsbäume. Ihre
  bestätigten Findings bleiben bis D3 hashgebunden, mit Owner und
  Re-Evaluation-Trigger erhalten.
...
- Keine direkte Level-1- oder Level-2-Änderung.
...
- **AC-DRG-009:** Jedes D1-Finding besitzt genau einen nachvollziehbaren
  Abschluss oder Handoff; Level-1-/Level-2-Findings sind vollständig und
  hashgebunden an D3 übergeben.
...
```text
$speckit-intake-review Review `Lastenheft_Level-0-Dokumentations-Konsistenz-Remediation-und-Flotten-Governance.md` as mandatory documentation preflight D2. Verify its strict dependency on the Level-0/1/2 D1 findings, complete level-specific disposition and D3 handoff, Level-0-only remediation boundary, the four exact Documentation Impact decisions, normative Constitution placement, template/checklist/PR integration, five-surface agent parity, deterministic versus semantic validation boundaries, learner accessibility, and no-product-change scope. Do not modify the intake or start downstream work.
```
...
```text
$speckit-specify Use `Lastenheft_Level-0-Dokumentations-Konsistenz-Remediation-und-Flotten-Governance.md` as the binding intake for mandatory documentation preflight D2 and the accepted Level-0/1/2 DOC### findings from D1 as mandatory evidence. Create the feature specification for central Level-0 documentation remediation and durable fleet Documentation Impact governance. Preserve DRG-001 through DRG-016, AC-DRG-001 through AC-DRG-009, the exact four impact decisions, complete level-specific finding disposition and D3 handoff, Constitution/template/checklist/PR/agent placement, generated-document ownership, deterministic validation boundaries, no speculative preset, no Level-1/2 writes, and all learner, A11Y, security, and non-goal constraints.
```
...
```text
$speckit-autonomous Execute the complete Spec Kit run for `Lastenheft_Level-0-Dokumentations-Konsistenz-Remediation-und-Flotten-Governance.md` with deliveryAuthority=MergeAndSync after documentation preflight D1 is complete. Remediate only accepted central Level-0 DOC### findings, disposition every D1 finding and preserve all Level-1/2 handoffs for D3, establish the approved fleet Documentation Impact contract across normative policy, templates, checklists, PR evidence, agent guidance, and deterministic gates, converge all required and optional checks and reviews, merge a non-empty Level-0 PR, return to clean synchronized main, and do not start preflight D3 automatically.
```
