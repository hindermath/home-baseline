...
- Optional: `autonomous-run-governance` v0.1.1 prio 70 wird nur bei explizit delegierten vollständigen Läufen über `scripts/config/spec-kit-autonomous-governance-presets.json` ergänzt. Die Standard-Sechsermatrix bleibt unverändert; Installation erteilt keine Remote-, Merge-, Bypass- oder Provider-Rechte und `LocalImplementation` bleibt Default. Dokumentations-, Status-, Schema- oder Evidence-Änderungen gelten erst dann als testfrei, wenn keine ausführbaren Validatoren die geänderten Pfade, Marker, Schemas oder Zustandswerte konsumieren.
