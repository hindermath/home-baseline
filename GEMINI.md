...
- Die Agenten-CLI-Oberflaechen `codex`, `claude` und `copilot` sind plattformuebergreifend Required und nutzen bei Bedarf die npm-Registry als Fallback. Google Antigravity ersetzt Gemini CLI; `agy` ist auf macOS per Homebrew Required, `Google.Antigravity` auf Windows per WinGet. Auf Linux bleibt Antigravity bis zu einem offiziell paketierten CLI-Pfad als begruendetes `N/A` dokumentiert.
