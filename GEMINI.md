...
- Die vier Agenten-CLI-Oberflaechen `codex`, `claude`, `gemini` und `copilot` sind Required. Fehlende `codex`-, `claude`-, `gemini`- und `copilot`-CLIs werden auf macOS, Linux und Windows ueber die npm-Registry als gemeinsamer Fallback installiert.
