...
- Optional: `autonomous-run-governance` v0.1.0 prio 70 wird nur bei explizit delegierten vollständigen Läufen über `scripts/config/spec-kit-autonomous-governance-presets.json` ergänzt. Die Standard-Sechsermatrix bleibt unverändert; Installation erteilt keine Remote-, Merge-, Bypass- oder Provider-Rechte und `LocalImplementation` bleibt Default.
